...
The pipeline runs inside Docker using docker-compose, and the Airflow UI can be accessed at [http://localhost:8081](http://localhost:8081) after starting the services with `docker compose up -d`. The default login is airflow / airflow. The Airflow environment requires a Snowflake connection named `open_meteo_api`, as well as Airflow Variables named `LATITUDE` and `LONGITUDE` for the location coordinates.
