...
def extract(city, longitude, latitude):
    ...
    file_path = f"/tmp/{safe_city}_{date_to_fetch}.csv"
...
def load(file_path, database, schema, target_table):
    ...
    cur = return_snowflake_conn("open_meteo_api")
    ...
    try:
        ...
        cur.execute(f"DELETE FROM {database}.{schema}.{target_table} WHERE date >= '{date_to_fetch}' AND date <= '{next_day_of_date_to_fetch}'")
...
with DAG(
    dag_id = 'weather_ETL_inc', # TO DO: add incremental as a suffix to the DAG id
    ...
    CITY = "San Jose"               # TO DO: Set the city name to your coordinates' city name
    
    database = "user_db_dolphin"           # TO DO: Set the database name  
    schema = "raw"
    target_table = "weather_data_hw6"       # TO DO: Set the target table name
